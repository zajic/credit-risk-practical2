DATA_FILEPATH = "dataset_imputed_final_holdout.csv"
WORKSPACE_IMAGE_PATH = "final.RData"
...
createConfusionMatrix <- function(act, pred) {
  ...
  print ("GOOD    PASS    BAD")
  print(paste(tpGood,"  ",good_pass,"   ",good_bad))
  print("0      0       0")
  print(paste(bad_good,"   ",bad_pass,"   ",tpBad))
...
testset <- read.csv("dataset_imputed_final_test.csv")
...
rf_model <- result$caretModel

predicted <- predict(rf_model,newdata = test_predictor,type="prob")
predicted$BAD <- NULL
...
library(pROC)
test_predictor$prob <- predict(rf_model,newdata=test_predictor,type="prob")$GOOD
